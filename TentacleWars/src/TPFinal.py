...
class CellWar(object):
    ...
    def tryConsiderCut(self):
        for chain in self.chains:
            if chain.color == GREEN and not chain.shouldGrow and not chain.shouldCollapse:
                ...
                intersect = self.findIntersection(self.initial[0], self.initial[1], self.recordPos[0],
                                                  self.recordPos[1], stx, sty, endx, endy)
                if intersect:
    ...
    def redrawAll(self):
        ...
        try:
            self.playShine(self.shinex, self.shiney, True)
        except:
            pass
        pygame.display.flip()
